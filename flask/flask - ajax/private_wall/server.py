...
def datetimecompare(value, format = '%H'):
    ...
    if result.days:# days
        return f'{result.days} day/s ago'
    if result.days > 30:# months
        return f'{int(result.days/30)} month/s ago'

    return f'{int(hours)} hour/s ago' # hours
...
def send_message(recipient_id):
    ...
    all_sent_messages = get_all_sent_messages()
    return f"{len(all_sent_messages)}"

@app.route("/deletemessage/<int:id>")
def delete_message(id):
    print("*"*10,get_message(id), id)
...
def first_name_checker():
    mysql = connectToMySQL("mydb")
    cuser_query = "SELECT * FROM users WHERE first_name = %(first_name)s;"
    data = {'first_name': request.form['first_name']}
    result = mysql.query_db(cuser_query, data)
    print(result)
    print("*"*10,request.form['first_name'])
    if result:
        return "_not_valid first name is already taken"
    return "_valid looks fine"
